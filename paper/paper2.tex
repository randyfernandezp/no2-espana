...
un rango práctico de \SI{72}{\kilo\metre}, que cae a \SI{26}{\kilo\metre} tras
...
proporciona un \textbf{rango práctico de \SI{72}{\kilo\metre}} en el sentido
...
\SI{72}{} a \textbf{\SI{26}{\kilo\metre}}.
...
\SI{72}{\kilo\metre} sobre la variable bruta \emph{sobreestima} la
...
La calibración de la incertidumbre se evalúa en la escala del modelo, que es
donde vive la varianza de kriging. El objetivo validado es
$\Yp{\loc^{*}}\mid\bm{Y}$ —una observación nueva, con $\pepita\bm{I}$
incluido— y no el campo latente $\Wp{\loc^{*}}\mid\bm{Y}$, que es lo que
corresponde al comparar contra mediciones de estaciones.

La cobertura empírica del intervalo nominal al \SI{95}{\percent} es
\num{0.910}, y la desviación típica de los residuos estandarizados
\textbf{$\mathrm{ESD} = \num{1.225}$} frente al valor esperado de \num{1}. Las
dos cifras son coherentes entre sí: con residuos de desviación \num{1.225} y un
intervalo construido con el cuantil \num{1.96}, la cobertura teórica es
$\Pr(|Z| < \num{1.96}/\num{1.225}) = \num{0.890}$.

El diagnóstico es, por tanto, que \textbf{la varianza de kriging subestima la
real en un factor $\num{1.225}^2 = \num{1.50}$}. La causa no es la
especificación del objetivo de predicción sino que $\varesp$, $\pepita$ y
$\escala$ se tratan como \emph{conocidos} cuando en realidad se estimaron de
los mismos datos: la varianza de kriging no propaga la incertidumbre de esos
estimadores. Es el argumento estándar a favor de la formulación bayesiana que
se discute en el trabajo futuro.
...
\begin{enumerate}[label=(\roman*), leftmargin=1.8em, itemsep=2pt]
  ...
  estaciones.
  \item Los intervalos de predicción están mal calibrados por construcción:
  cobertura \num{0.910} y $\mathrm{ESD}=\num{1.225}$, es decir una varianza un
  \SI{50}{\percent} menor de la real. Los intervalos de este trabajo deben
  leerse como \emph{condicionales} a $\hat\varesp$, $\hat\pepita$ y
  $\hat\escala$.
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.6em, itemsep=4pt]
  ...
  de \SI{72}{\kilo\metre} que cae a \SI{26}{\kilo\metre} tras retirar la
